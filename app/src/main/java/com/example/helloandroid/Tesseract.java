package com.example.helloandroid;

import android.content.Context;
import android.content.res.AssetManager;
import android.graphics.Bitmap;
import android.util.Log;
import android.widget.Toast;

import com.googlecode.tesseract.android.TessBaseAPI;

import java.io.File;
import java.io.FileOutputStream;
import java.io.InputStream;


public class Tesseract {

    private static final String TAG = "Tesseract";

    private final TessBaseAPI mTess;

    public Tesseract(Context context, String language) {
        mTess = new TessBaseAPI();
        boolean fileExistFlag = false;

        String dstInitPathDir = context.getFilesDir() + "/tesseract";
        String dstPathDir = dstInitPathDir + "/tessdata/";
        String srcFile = "eng.traineddata";
        String dstPathFile = dstPathDir + srcFile;

        InputStream inFile = null;
        FileOutputStream outFile = null;

        AssetManager assetManager = context.getAssets();
        try {
            inFile = assetManager.open(srcFile);

            File file = new File(dstPathDir);

            if (!file.exists()) {
                if (!file.mkdirs()) {
                    Toast.makeText(context, srcFile + " cannot be created.",
                                   Toast.LENGTH_SHORT).show();
                }
                outFile = new FileOutputStream(new File(dstPathFile));
            } else {
                fileExistFlag = true;
            }
        } catch (Exception exception) {
            Log.e(TAG, exception.getMessage());
        } finally {
            if (fileExistFlag) {
                try {
                    if (inFile != null) inFile.close();
                    mTess.init(dstInitPathDir, language);
                    return;
                } catch (Exception exception) {
                    Log.e(TAG, exception.getMessage());
                }
            }

            if (inFile != null && outFile != null) {
                try {
                    //copy file
                    byte[] buf = new byte[1024];
                    int len;
                    while ((len = inFile.read(buf)) != -1) {
                        outFile.write(buf, 0, len);
                    }
                    inFile.close();
                    outFile.close();
                    mTess.init(dstInitPathDir, language);
                } catch (Exception ex) {
                    Log.e(TAG, ex.getMessage());
                }
            } else {
                Toast.makeText(context, srcFile + " cannot be read.",
                               Toast.LENGTH_SHORT).show();
            }
        }
    }

    public String getOCRResult(Bitmap bitmap) {
        mTess.setImage(bitmap);
        return mTess.getUTF8Text();
    }

    public void onDestroy() {
        if (mTess != null) mTess.end();
    }
}
